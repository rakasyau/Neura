...
function DaftarForm() {
  ...
  const handleGoogleCredential = async (response: { credential: string }) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        notify("Pendaftaran Berhasil!", `Selamat datang di Neura, ${data.user.name}!`, "achievement")
        router.push(redirectTarget)
        router.refresh()
      } else {
        setErrorMsg(data.error || "Gagal autentikasi Google.")
        notify("Gagal Mendaftar", data.error || "Gagal autentikasi Google.", "error")
      }
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !email || !password) {
      setErrorMsg("Mohon isi seluruh kolom pendaftaran")
      return
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        const msg = data.error || "Gagal mendaftar akun baru."
        setErrorMsg(msg)
        notify("Gagal Mendaftar", msg, "error")
      }
    } catch {
      setErrorMsg("Terjadi kendala jaringan saat mencoba mendaftar.")
    } finally {
      setLoading(false)
    }
  }
    ...
    <>
      <GlassCard className="p-8 sm:p-10 border border-white/15 shadow-2xl relative overflow-hidden">
        <div className="text-center mb-8">
          ...
          <p className="text-xs sm:text-sm text-neura-muted">Mulai Belajar ML & Akses AI Assistant</p>
        </div>
